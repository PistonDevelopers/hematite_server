...
pub trait PacketWrite {
    ...
    fn inner_encode(&self, dst: &mut dyn Write) -> io::Result<()>;
    ...
    fn write(&self, dst: &mut dyn Write) -> io::Result<()> {
        ...
        <Var<i32> as Protocol>::proto_encode(&(len as i32), dst)?;
...
pub trait PacketRead: Sized {
    fn inner_decode(src: &mut dyn Read) -> io::Result<Self>;

    /// Reads a new packet from a reader, including length.
    ///
    /// **TODO:** add support for compression.
    fn read<R: Read>(src: &mut R) -> io::Result<Self> {
        let proto_len = <Var<i32> as Protocol>::proto_decode(src)?;
...
mod prelude {
    ...
    pub use crate::packet::{
        BlockChangeRecord, ChunkMeta, NextState, PacketRead, PacketWrite, Protocol, Stat,
    };
    pub use crate::proto::slp;
    pub use crate::types::consts::*;
    pub use crate::types::{Arr, BlockPos, ChunkColumn, Slot, UuidString, Var};
}
...
macro_rules! impl_protocol {
    ($name:ty, 1, $enc_name:ident, $dec_name:ident) => {
        impl Protocol for $name {
            ...
            fn proto_decode(src: &mut dyn Read) -> io::Result<$name> {
                src.$dec_name().map_err(io::Error::from)
            }
        }
    };
    ($name:ty, $len:expr, $enc_name:ident, $dec_name:ident) => {
        impl Protocol for $name {
            ...
            fn proto_decode(src: &mut dyn Read) -> io::Result<$name> {
                src.$dec_name::<BigEndian>().map_err(io::Error::from)
            }
        }
    };
}
...
macro_rules! proto_struct {
    ...
    ($name:ident { $($fname:ident: $fty:ty),+ }) => {
        ...
        impl Protocol for $name {
            ...
            fn proto_encode(value: &$name, dst: &mut dyn Write) -> io::Result<()> {
                $((<$fty as Protocol>::proto_encode(&value.$fname, dst))?;)*
                Ok(())
            }

            fn proto_decode(src: &mut dyn Read) -> io::Result<$name> {
                Ok($name {
                    $($fname: (<$fty as Protocol>::proto_decode(src))?),*
                })
            }
        }
    };
    ...
    ($name:ident {}) => {
        ...
        impl Protocol for $name {
            ...
            fn proto_encode(_: &Self, _: &mut dyn Write) -> io::Result<()> {
                Ok(())
            }

            fn proto_decode(_: &mut dyn Read) -> io::Result<$name> {
                Ok($name)
            }
        }
    };
...
impl<T: Protocol> Protocol for Option<T> {
    ...
    fn proto_encode(value: &Option<T::Clean>, dst: &mut dyn Write) -> io::Result<()> {
        match *value {
            Some(ref inner) => {
                <bool as Protocol>::proto_encode(&true, dst)?;
                <T as Protocol>::proto_encode(inner, dst)?;
            }
            None => {
                <bool as Protocol>::proto_encode(&false, dst)?;
            }
        }
        Ok(())
    }

    fn proto_decode(src: &mut dyn Read) -> io::Result<Option<T::Clean>> {
        if <bool as Protocol>::proto_decode(src)? {
            Ok(Some(<T as Protocol>::proto_decode(src)?))
        } else {
            Ok(None)
        }
    }
            ...
            0x26 => ChunkDataBulk { sky_light_sent: bool, chunk_meta: Vec<ChunkMeta>, chunk_data: Vec<ChunkColumn>;
                impl Protocol for ChunkDataBulk {
                    ...
                    fn proto_len(this: &Self) -> usize {
                        ...
                        + this.chunk_data.iter().map(ChunkColumn::len).fold(0, |acc, item| acc + item)
                    }
                    ...
                    fn proto_decode(src: &mut dyn Read) -> io::Result<ChunkDataBulk> {
                        ...
                        for cm in &mut chunk_meta {
                            *cm = <ChunkMeta as Protocol>::proto_decode(src)?;
                        }
                        // Read all encoded ChunkColumns, buffer size starts at 4 KB, probably will get bigger
                        ...
                        for (cd, cm) in chunk_data.iter_mut().zip(chunk_meta.iter()) {
                            ...
                            *cd = ChunkColumn::decode(&mut src, cm.mask, true, true)?;
                        }
                        Ok(ChunkDataBulk{
                            sky_light_sent,
                            chunk_meta,
