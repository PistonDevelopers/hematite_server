...
use crate::types::consts::Color;
use crate::types::selector;
use crate::types::EntitySelector;
    ...
    pub fn from_json(json: Json) -> Result<ChatJson, ChatJsonError> {
        match json {
                    ...
                    match &key[..] {
                        ...
                        "with" => {
                            type_check!(&key => value, Array(with_json) {
                                let with = r#try!(with_json.into_iter().map(ChatJson::from_json).collect());
                        ...
                        "extra" => {
                            type_check!(&key => value, Array(extra) {
                                result.extra = r#try!(extra.into_iter().map(ChatJson::from_json).collect());
                            });
                        }
            ...
            Json::Array(array) => Ok(ChatJson {
                extra: r#try!(array.into_iter().map(ChatJson::from_json).collect()),
                ..ChatJson::from("")
            }),
